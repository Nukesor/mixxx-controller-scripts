...
TraktorS2MK3.fxHandler = function (field) {
    ...
    // Disable effects on all other effect units.
    for (var i = 1; i <= 4; ++i) {
        // Don't set the effects on the currently pressed unit
        if (i == fxNumber) {
            continue;
        }
        // The group is already disabled. Don't do anything.
        if (!TraktorS2MK3.fxButtonState[i]) {
            continue;
        }

        TraktorS2MK3.fxButtonState[i] = false;
        var group = "[EffectRack1_EffectUnit" + i + "]";
        engine.setValue(group, "group_[Channel1]_enable", TraktorS2MK3.fxButtonState[i]);
        engine.setValue(group, "group_[Channel2]_enable", TraktorS2MK3.fxButtonState[i]);
        TraktorS2MK3.outputHandler(TraktorS2MK3.fxButtonState[i], field.group, "fxButton" + i);
    }
